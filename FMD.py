...
class FMD:
    ...
    """
    """ class_infos
    class_infos: dir_infos, data_infos, fixed_FM_repres, fixed_alpha_infos,
                fixed_layer_infos 모두를 통틀어 class_infos라고 한다.
    """
    ...
    FM_repre_MHP:
    평가하고 싶은 FM_repre 종류 모두를 지정한다.
    FM_repre_MHP는 'mean', 'min', 'max'가 있다.
    
    alpha_MHP:
    rmw_max로 할지, 특정 alpha 값들로 할지 지정할 수 있다.
    Ex. alpha_MHP = [['rmw', 100], [1,2,3,4,5,6]]
    
    DAM_MHP:
    평가하고 싶은 DAM 종류 모두를 지정한다.
    DAM_MHP은 'and', 'or', 'wfm', 'all'이 있다.
    
    W_MHP:
    평가하고 싶은 W 종류 모두를 지정한다.
    W_MHP은 'C'=constant, 'I'=increasing 있다.
    
    lfmd_MHP:
    평가하고 싶은 lfmd 종류 모두를 지정한다.
    lfmd_MHP은 'se_lfmd', 'Ln_lfmd'가 있다.
    
    fmdc_MHP:
    평가하고 싶은 fmdc 종류 모두를 지정한다.
    fmdc_MHP는 'rvalid_fmds_max', 'rvalid_fmds_average', 'rvalid_fmds_middle',
    'wvalid_fmds_min', 'wvalid_fmds_average', 'wvalid_fmds_middle',
    'rvalid_fmds_max_wvalid_fmds_min_average'
    (M: Max, m: min, A:Average, mid: middle)가 있다.
    """
    metrics = {}
    metric_names = []
    """ metric_infos
    ...
    def set_data_infos(self):
        ...
        for eval_name in self.eval_names:
            self.eval_U[eval_name] = np.load(
                f"{self.eval_dir}/{eval_name}/{eval_name}_eval_U.npy"
            )

    def set_FM_repres(self):
        # 인스턴스 속성을 변수로 포인터처럼 가르킴
        train = self.origin_names[0]
        rvalid = self.origin_names[1]
        wvalid = self.origin_names[2]
        L = self.L
        shape = self.shape
        ...
        def set_FM_repre(origin):
            ...
            if origin == train:
                OFM_repre = self.TFM_repre
                origin_dir = self.train_dir
            elif origin == rvalid:
                OFM_repre = self.RFM_repre
                origin_dir = self.rvalid_dir
            elif origin == wvalid:
                OFM_repre = self.WFM_repre
                origin_dir = self.wvalid_dir
            else:
    ...
    def set_alpha_rmw_min_max(self):
        ...
        # alpha_min, alpha_max
        self.alpha_min["FM_min"] = []
        self.alpha_max["FM_min"] = []
        self.alpha_min["FM_mean"] = []
        self.alpha_max["FM_mean"] = []
        self.alpha_min["FM_max"] = []
        self.alpha_max["FM_max"] = []
        # rmw_min, rmw_max
        self.rmw_min = []
        self.rmw_max = []
        ...
        for l in range(self.L):
            self.alpha_min["FM_min"].append(
                np.array(
                    [
                        self.RFM_repre["FM_min"][l].min(),
                        self.TFM_repre["FM_min"][l].min(),
                        self.WFM_repre["FM_min"][l].min(),
                    ]
                ).min()
            )
            self.alpha_max["FM_min"].append(
                np.array(
                    [
                        self.RFM_repre["FM_min"][l].max(),
                        self.TFM_repre["FM_min"][l].max(),
                        self.WFM_repre["FM_min"][l].max(),
                    ]
                ).max()
            )

            self.alpha_min["FM_mean"].append(
                np.array(
                    [
                        self.RFM_repre["FM_mean"][l].min(),
                        self.TFM_repre["FM_mean"][l].min(),
                        self.WFM_repre["FM_mean"][l].min(),
                    ]
                ).min()
            )
            self.alpha_max["FM_mean"].append(
                np.array(
                    [
                        self.RFM_repre["FM_mean"][l].max(),
                        self.TFM_repre["FM_mean"][l].max(),
                        self.WFM_repre["FM_mean"][l].max(),
                    ]
                ).max()
            )

            self.alpha_min["FM_max"].append(
                np.array(
                    [
                        self.RFM_repre["FM_max"][l].min(),
                        self.TFM_repre["FM_max"][l].min(),
                        self.WFM_repre["FM_max"][l].min(),
                    ]
                ).min()
            )
            self.alpha_max["FM_max"].append(
                np.array(
                    [
                        self.RFM_repre["FM_max"][l].max(),
                        self.TFM_repre["FM_max"][l].max(),
                        self.WFM_repre["FM_max"][l].max(),
                    ]
                ).max()
            )
        ...
        for l in range(self.L):
            ...
            self.rmw_min.append(-shape_l_count)
            self.rmw_max.append(shape_l_count)
        # rmw_min, rmw_max 넘파이로 변경
        self.rmw_min = np.array(self.rmw_min)
        self.rmw_max = np.array(self.rmw_max)
    ...
    def set_MHP(
        ...
        # instance 하나 당 2MB일 때, limited_number: metrics의 총 용량이 1GB가 되는 개수.
        limited_number = 512
        number_of_all_case = (
            len(FM_repre_MHP)
            * len(alpha_MHP)
        ...
        # * MHP 초기화
        self.FM_repre_MHP = FM_repre_MHP
        self.alpha_MHP = alpha_MHP
        self.DAM_MHP = DAM_MHP
        self.W_MHP = W_MHP
        self.lfmd_MHP = lfmd_MHP
        self.fmdc_MHP = fmdc_MHP
    ...
    def init_metrics(self):
        ...
        for i in range(len(self.alpha_MHP)):
            ...
            if self.alpha_MHP[i][0] == "rmw_max":
                alpha_MHP_str.append(
                    str(i)
                    + ","
                    + self.alpha_MHP[i][0]
                    + ","
                    + str(self.alpha_MHP[i][1])
                )
            else:
                alpha_MHP_i_str = ""
        ...
        for i, metric_name_list in enumerate(self.metric_names):
            ...
            FM_repre_HP = metric_name_list[0]
            alpha_HP_str = metric_name_list[1]
            DAM_HP = metric_name_list[2]
            lfmd_HP = metric_name_list[3]
            W_HP = metric_name_list[4]
            fmdc_HP = metric_name_list[5]
        ...
        for i, metric_name in enumerate(self.metric_names):
            ...
            """ FM_repre_infos
            FM_repre_HP:
            FM_repre를 어떤 것으로 선택할지 정하는 HP, FM_min, FM_mean, FM_max가 있다.
            """
            ...
            if alpha_HP_list[1] == "rmw_max":
                self.metrics[metric_name]["alpha_slice"] = self.alpha_MHP[
                    alpha_MHP_index
                ][1]
                for l in range(self.L):
            ...
            # alpha를 넘파이로 변경
            self.metrics[metric_name]["alpha"] = np.array(
                self.metrics[metric_name]["alpha"], dtype="float"
            )
            """ alpha_infos
            1. alpha_slice:
            alpha_min에서 alpha_max로 몇 번의의 간격으로 도착할지 알려주는 변수임.
            2. alpha:
            거리 계산을 위한 인덱스를 고르기 위해 필요한 변수이다.
            3. alpha_percent:
            alpha_min, alpha_max 사이에 alpha가 위치한 곳이다.
            4. rmw:
            훈련과 정분류이 비슷하고 훈련과 오분류가 비슷하지 않을수록 값이 커진다.
            5. rmw_percent:
            rmw_min, rmw_max 사이에 rmw가 위치한 곳이다.
            """

            # * 3.5. AMs
            self.metrics[metric_name]["TAM"] = []
            self.metrics[metric_name]["RAM"] = []
            self.metrics[metric_name]["WAM"] = []
            ...
            # * 3.6. DAM_infos
            self.metrics[metric_name]["DAM_indexes"] = []
            self.metrics[metric_name]["DAM"] = []
            self.metrics[metric_name]["DAM_HP"] = DAM_HP
            self.metrics[metric_name]["DAM_error_flag"] = []
            """ DAM_infos
            1. DAM_indexes:
            나중에 거리 계산할 때 쓰이는 다차원 인덱스들의 집합이다.
            각 원소는 피처 맵의 한 원소의 인덱스를 나타낸다.
            각 레이어마다 튜플들 세트가 있어야 함. np.array의 item 메소드를 사용할 것이기 때문
            2. DAM:
            거리 활성화 맵.
            DAM는 거리 계산을 위한 인덱스만 활성된 맵이다.
            3. DAM_HP:
            DAM를 고르는 방법을 알려줌.
            4. DAM_error_flag:
            예외 처리된 경우.
            0은 예외 처리되지 않음.
            1은 예외 처리되어 WFM 방식을 택함.
            2는 예외 처리되어 모든 인덱스를 택함.
            """
            ...
            if W_HP == "C":
                ...
                for l in range(self.L):
                    self.metrics[metric_name]["W"].append(
                        (l + 1) * (2 / (self.L * (self.L + 1)))
                    )
            ...
            # * 3.8. fmdc_infos
            self.metrics[metric_name]["fmdc"] = -1
            self.metrics[metric_name]["fmdc_HP"] = fmdc_HP
            self.metrics[metric_name]["HP_fmdcs"] = {}
            self.metrics[metric_name]["rvalid_fmds"] = []
            self.metrics[metric_name]["wvalid_fmds"] = []
            fmdc_HPs = [
                "rvalid_fmds_max",
                "rvalid_fmds_average",
                "rvalid_fmds_middle",
            ...
            # * 3.9. eval_infos
            # eval_U도 포함.
            self.metrics[metric_name]["is_eval_FMD"] = {}
            self.metrics[metric_name]["eval_fmds"] = {}
            self.metrics[metric_name]["TP"] = {}
            self.metrics[metric_name]["FN"] = {}
            self.metrics[metric_name]["TN"] = {}
            self.metrics[metric_name]["FP"] = {}
            self.metrics[metric_name]["P"] = {}
            self.metrics[metric_name]["N"] = {}
            self.metrics[metric_name]["TPR"] = {}
            self.metrics[metric_name]["TNR"] = {}
            self.metrics[metric_name]["PPV"] = {}
            self.metrics[metric_name]["NPV"] = {}
            self.metrics[metric_name]["FNR"] = {}
            self.metrics[metric_name]["FPR"] = {}
            self.metrics[metric_name]["FDR"] = {}
            self.metrics[metric_name]["FOR"] = {}
            for eval_name in self.eval_names:
                self.metrics[metric_name]["is_eval_FMD"][eval_name] = []
                self.metrics[metric_name]["eval_fmds"][eval_name] = []
                self.metrics[metric_name]["TP"][eval_name] = -1
                self.metrics[metric_name]["FN"][eval_name] = -1
                self.metrics[metric_name]["TN"][eval_name] = -1
                self.metrics[metric_name]["FP"][eval_name] = -1
                self.metrics[metric_name]["P"][eval_name] = -1
                self.metrics[metric_name]["N"][eval_name] = -1
                self.metrics[metric_name]["TPR"][eval_name] = -1
                self.metrics[metric_name]["TNR"][eval_name] = -1
                self.metrics[metric_name]["PPV"][eval_name] = -1
                self.metrics[metric_name]["NPV"][eval_name] = -1
                self.metrics[metric_name]["FNR"][eval_name] = -1
                self.metrics[metric_name]["FPR"][eval_name] = -1
                self.metrics[metric_name]["FDR"][eval_name] = -1
                self.metrics[metric_name]["FOR"][eval_name] = -1

            """ eval_infos
            1. eval_U:
            평가 데이터의 정분류(True), 오분류(False) 유무를 True, False로 담는다
            2. is_eval_FMD:
            평가 데이터가 is_eval_FMD이면 True, eval_fmd가 아니면 False를 담는다.
            3. eval_fmds:
            평가 데이터들의 fmd를 담는다.
            ...
            TPR, TNR, PPV, NPV,
            FNR, FPR, FDR, FOR:
            => confusion matrix를 표현하기 위한 가장 기본적인 속성이다.
            """

            # * 3.10. fmdcs_infos
            self.metrics[metric_name]["fmdcs"] = {}
            self.metrics[metric_name]["TP_fmdc"] = {}
            self.metrics[metric_name]["FN_fmdc"] = {}
            self.metrics[metric_name]["TN_fmdc"] = {}
            self.metrics[metric_name]["FP_fmdc"] = {}
            self.metrics[metric_name]["P_fmdc"] = {}
            self.metrics[metric_name]["N_fmdc"] = {}
            self.metrics[metric_name]["TPR_fmdc"] = {}
            self.metrics[metric_name]["TNR_fmdc"] = {}
            self.metrics[metric_name]["PPV_fmdc"] = {}
            self.metrics[metric_name]["NPV_fmdc"] = {}
            self.metrics[metric_name]["FNR_fmdc"] = {}
            self.metrics[metric_name]["FPR_fmdc"] = {}
            self.metrics[metric_name]["FDR_fmdc"] = {}
            self.metrics[metric_name]["FOR_fmdc"] = {}
            self.metrics[metric_name]["AUC"] = {}
            for eval_name in self.eval_names:
                self.metrics[metric_name]["fmdcs"][eval_name] = []
                self.metrics[metric_name]["TP_fmdc"][eval_name] = []
                self.metrics[metric_name]["FN_fmdc"][eval_name] = []
                self.metrics[metric_name]["TN_fmdc"][eval_name] = []
                self.metrics[metric_name]["FP_fmdc"][eval_name] = []
                self.metrics[metric_name]["P_fmdc"][eval_name] = []
                self.metrics[metric_name]["N_fmdc"][eval_name] = []
                self.metrics[metric_name]["TPR_fmdc"][eval_name] = []
                self.metrics[metric_name]["TNR_fmdc"][eval_name] = []
                self.metrics[metric_name]["PPV_fmdc"][eval_name] = []
                self.metrics[metric_name]["NPV_fmdc"][eval_name] = []
                self.metrics[metric_name]["FNR_fmdc"][eval_name] = []
                self.metrics[metric_name]["FPR_fmdc"][eval_name] = []
                self.metrics[metric_name]["FDR_fmdc"][eval_name] = []
                self.metrics[metric_name]["FOR_fmdc"][eval_name] = []
                self.metrics[metric_name]["AUC"][eval_name] = -1

            """ fmdcs_infos
            * eval_infos를 구하고 난 후에 구하는 값들이다.
            fmdcs:
            eval_fmds를 density(=1000)만큼 조각 내서 구한 fmdc를 모두 담는다.
            # ? CM_info는 TP, FN, TN, FN, N, P, TPR, TNR, PPV, NPV, FNR, FPR, FDR, FOR을
            # ? 대명사로 표현할 때 쓰인다.
            ...
            FNR_fmdc, FPR_fmdc, FDR_fmdc, FOR_fmdc,:
            fmdcs의 원소마다 14개 배열의 각 원소를 구한다.
            
            AUC:
            위 TPR_fmdc, TNRs_fmdc을 가지고 만든 ROC curve의 아래 부분과 y=0, x=1 사이의 넓이이다.
            """
    ...
    def set_alpha_infos_and_AMs_and_DAM_infos(self, metric_name):
        ...
        # * alpha_percent, rmw_percent에 L 크기 만큼 -1로 초기화한다.
        for l in range(self.L):
        ...
        if alpha_HP_list[0] == "rmw_max":
            ...
            for l in range(self.L):
                alpha_min_l = self.alpha_min[FM_repre_HP][l]
                alpha_max_l = self.alpha_max[FM_repre_HP][l]
                alpha_interval_l = (alpha_max_l - alpha_min_l) / alpha_slice
                # range(a_slice_l+1) 해야 a_min_l 부터 a_max_l 까지 감
                for alpha_interval_offset_l, alpha_l in enumerate(
                    [
                        alpha_min_l + alpha_interval_l * alpha_interval_offset_l
                        for alpha_interval_offset_l in range(alpha_slice + 1)
                    ]
                ):
                    ...
                    rmw_min_l = self.rmw_min[l]
                    rmw_max_l = self.rmw_max[l]
                    if alpha_interval_offset_l == 0:
                        ...
                        alpha_percent_l = np.round(
                            ((alpha_l - alpha_min_l) / (alpha_max_l - alpha_min_l))
                            * 100
                        )
                        ...
                        rmw_percent_l = np.round(
                            (
                                (self.metrics[metric_name]["rmw"][l] - rmw_min_l)
                                / (rmw_max_l - rmw_min_l)
                            )
                            * 100
                        )
                    ...
                    # alpha, r-w, TAM, RAM, WAM를 최신화함.
                        ...
                        alpha_interval_offset_l > 0
                        and r_l - w_l > self.metrics[metric_name]["rmw"][l]
                    ):
                        self.metrics[metric_name]["alpha"][l] = alpha_l
                        # alpha_min이 0%이고 alpha_max가 100%일 때 alpha_percent은 alpha의 위치가 몇 퍼센트인지 알려줌.
                        alpha_percent_l = np.round(
        ...
        # * 2. 특정 alpha 값들을 선택하는 방식일 경우, 문자열이 '숫자'일 경우
            ...
            # alpha_percent을 구함
            alpha_min = self.alpha_min[FM_repre_HP]
            alpha_max = self.alpha_max[FM_repre_HP]
            # alpha_min이 0%이고 alpha_max가 100%일 때 alpha_percent은 alpha의 위치가 몇 퍼센트인지 알려줌.
            alpha_percent = np.round(
                ((alpha - alpha_min) / (alpha_max - alpha_min)) * 100
            )
            ...
            for l in range(self.L):
                ...
                self.metrics[metric_name]["TAM"][l] = np.array(
                    self.TFM_repre[FM_repre_HP][l] > alpha[l]
                )
                self.metrics[metric_name]["RAM"][l] = np.array(
                    self.RFM_repre[FM_repre_HP][l] > alpha[l]
                )
                self.metrics[metric_name]["WAM"][l] = np.array(
                    self.WFM_repre[FM_repre_HP][l] > alpha[l]
                )

                TAM_l_xnor_RAM_l = np.logical_not(
                    np.logical_xor(
                        self.metrics[metric_name]["TAM"][l],
                        self.metrics[metric_name]["RAM"][l],
                    )
                )
                TAM_l_xnor_WAM_l = np.logical_not(
                    np.logical_xor(
                        self.metrics[metric_name]["TAM"][l],
                        self.metrics[metric_name]["WAM"][l],
                    )
                )
        ...
        TAM = self.metrics[metric_name]["TAM"]
        RAM = self.metrics[metric_name]["RAM"]
        WAM = self.metrics[metric_name]["WAM"]
        ...
        for l in range(self.L):
            ...
            if (
                len(np.where(self.metrics[metric_name]["DAM"][l] == False)[0])
                == shape_l_size
            ):
        ...
        for l in range(self.L):
            ...
            for i in range(len(nonzero_DAM_l)):
                DAM_indexes_l = np.append(
                    DAM_indexes_l, nonzero_DAM_l[i].reshape(1, -1), axis=0
                )
    ...
    def se_lfmd(self, metric_name, FM_k_l, l, percent=50, sensitive=1):
        ...
        TFM_repre_l_norm = self.normalize_layer(
            self.TFM_repre[FM_repre_HP][l], norm_min, norm_max
        )
        ...
        for index in self.metrics[metric_name]["DAM_indexes"][l]:
            ...
            if 0.5 <= lengths.item(index) and lengths.item(index) <= 1:
                exp_lengths_minus_shift_value.itemset(
                    index,
                    (np.exp(lengths.item(index) - shift_value)) ** sensitive
                    + np.exp(0.5) ** sensitive
                    - 2,
                )
            else:
                exp_lengths_minus_shift_value.itemset(
    ...
    def fmd(self, metric_name, FM_k):
        ...
        for l in range(self.L):
            FM_k_l = FM_k[l]
            lfmd_l = self.lfmd(self.metrics[metric_name]["lfmd_HP"])(
                metric_name, FM_k_l, l
            )
            lfmds.append(lfmd_l)
    ...
    def set_fmds(self, metric_name):
        def set_fmds(metric_name, valid_name):
            ...
            if valid_name == "rvalid":
                valid = self.origin_names[1]
                valid_dir = self.rvalid_dir
                valid_K = self.origin_K[valid]
                FMP_k = None
                prev_FMPI_k = None
                cur_FMPI_k = None
                fmds = self.metrics[metric_name]["rvalid_fmds"]
            elif valid_name == "wvalid":
                valid = self.origin_names[2]
                valid_dir = self.wvalid_dir
                valid_K = self.origin_K[valid]
                FMP_k = None
                prev_FMPI_k = None
                cur_FMPI_k = None
                fmds = self.metrics[metric_name]["wvalid_fmds"]
        ...
        rvalid = self.origin_names[1]
        wvalid = self.origin_names[2]
        set_fmds(metric_name, rvalid)
        self.metrics[metric_name]["rvalid_fmds"] = np.array(
            self.metrics[metric_name]["rvalid_fmds"]
        )
        set_fmds(metric_name, wvalid)
        self.metrics[metric_name]["wvalid_fmds"] = np.array(
            self.metrics[metric_name]["wvalid_fmds"]
        )
    ...
    def set_fmdc(self, metric_name):
        ...
        self.metrics[metric_name]["HP_fmdcs"]["rvalid_fmds_middle"] = rvalid_fmds[
            len(rvalid_fmds) // 2
        ]
        ...
        self.metrics[metric_name]["HP_fmdcs"]["wvalid_fmds_middle"] = wvalid_fmds[
            len(wvalid_fmds) // 2
        ]
        self.metrics[metric_name]["HP_fmdcs"][
            "rvalid_fmds_max_wvalid_fmds_min_average"
        ] = (rvalid_fmds.max() + wvalid_fmds.min()) / 2
        # * fmdc_HP 방식대로 fmdc 지정하기
        # * 정분류에 대한 fmdc
        if fmdc_HP == "rvalid_fmds_max":
            self.metrics[metric_name]["fmdc"] = self.metrics[metric_name]["HP_fmdcs"][
                "rvalid_fmds_max"
            ]
    ...
    def fit(
        ...
        if os.path.isfile(f"{class_dir}/class_infos.pickle"):
            with open(f"{class_dir}/class_infos.pickle", "rb") as class_infos_file:
                ...
                self.class_dir = class_infos["class_dir"]
                self.origin_dir = class_infos["origin_dir"]
                self.train_dir = class_infos["train_dir"]
                self.rvalid_dir = class_infos["rvalid_dir"]
                self.wvalid_dir = class_infos["wvalid_dir"]
                self.eval_dir = class_infos["eval_dir"]
                # * 1.1.2. data_infos 초기화하기
                self.origin_names = class_infos["origin_names"]
                self.origin_K = class_infos["origin_K"]
                self.eval_names = class_infos["eval_names"]
                self.eval_K = class_infos["eval_K"]
                self.L = class_infos["L"]
                self.shape = class_infos["shape"]
                self.FMP_count = class_infos["FMP_count"]
                self.eval_U = class_infos["eval_U"]
                # * 1.1.3. fixed_FM_repres 초기화하기
                self.TFM_repre = class_infos["TFM_repre"]
                self.RFM_repre = class_infos["RFM_repre"]
                self.WFM_repre = class_infos["WFM_repre"]
                # * 1.1.4. fixed_alpha_infos 초기화하기
                self.alpha_min = class_infos["alpha_min"]
                self.alpha_max = class_infos["alpha_max"]
                self.rmw_min = class_infos["rmw_min"]
                self.rmw_max = class_infos["rmw_max"]
                # * 1.1.5. fixed_layer_infos 초기화하기
                self.norm_min = class_infos["norm_min"]
                self.norm_max = class_infos["norm_max"]
        ...
        for metric_name in self.metric_names:
            ...
            if os.path.isfile(f"{self.class_dir}/metrics/{metric_name}.pickle"):
                with open(
                    f"{self.class_dir}/metrics/{metric_name}.pickle", "rb"
                ) as metric_file:
                    metric = pickle.load(metric_file)

                    # * 3.1.1 alpha_infos 초기화
                    self.metrics[metric_name]["alpha_slice"] = metric["alpha_slice"]
                    self.metrics[metric_name]["alpha"] = metric["alpha"]
                    self.metrics[metric_name]["alpha_percent"] = metric["alpha_percent"]
                    self.metrics[metric_name]["rmw"] = metric["rmw"]
                    self.metrics[metric_name]["rmw_percent"] = metric["rmw_percent"]
                    # * 3.1.2 AMs 초기화
                    self.metrics[metric_name]["TAM"] = metric["TAM"]
                    self.metrics[metric_name]["RAM"] = metric["RAM"]
                    self.metrics[metric_name]["WAM"] = metric["WAM"]
                    # * 3.1.3 DAM_infos 초기화
                    self.metrics[metric_name]["DAM_indexes"] = metric["DAM_indexes"]
                    self.metrics[metric_name]["DAM"] = metric["DAM"]
                    self.metrics[metric_name]["DAM_HP"] = metric["DAM_HP"]
                    self.metrics[metric_name]["DAM_error_flag"] = metric[
                        "DAM_error_flag"
                    ]
                    # * 3.1.4 fmdc_infos 초기화
                    self.metrics[metric_name]["fmdc"] = metric["fmdc"]
                    self.metrics[metric_name]["fmdc_HP"] = metric["fmdc_HP"]
                    self.metrics[metric_name]["HP_fmdcs"] = metric["HP_fmdcs"]
                    self.metrics[metric_name]["rvalid_fmds"] = metric["rvalid_fmds"]
                    self.metrics[metric_name]["wvalid_fmds"] = metric["wvalid_fmds"]
    ...
    def set_eval_infos(self, metric_name):
        ...
        for eval_name in self.eval_names:
            ...
            eval_dir = f"{self.eval_dir}/{eval_name}"
            eval_K = self.eval_K[eval_name]
            EFMP_k = None
            prev_EFMPI_k = None
            cur_EFMPI_k = None
            ...
            for k in range(eval_K):
                ...
                # cur_EFMPI_k와 prev_EFMPI_k가 다를 경우
                    ...
                    with open(
                        f"{eval_dir}/{eval_name}_{cur_EFMPI_k}.pickle", "rb"
                    ) as f:
                ...
                self.metrics[metric_name]["is_eval_FMD"][eval_name].append(
                    fmd >= self.metrics[metric_name]["fmdc"]
                )
            ...
            self.metrics[metric_name]["is_eval_FMD"][eval_name] = np.array(
                self.metrics[metric_name]["is_eval_FMD"][eval_name]
            )
            self.metrics[metric_name]["eval_fmds"][eval_name] = np.array(
                self.metrics[metric_name]["eval_fmds"][eval_name]
            )
        ...
        for eval_name in self.eval_names:
            ...
            is_eval_FMD = self.metrics[metric_name]["is_eval_FMD"][eval_name]
            eval_FMD_K = len(eval_U[is_eval_FMD])
            eval_FMD_r = len(np.nonzero(eval_U[is_eval_FMD])[0])
            eval_FMD_w = eval_FMD_K - eval_FMD_r

            # * 3.2. TP, FN, TN, FP 초기화
            self.metrics[metric_name]["TP"][eval_name] = (
                eval_U_r - eval_FMD_r
            ) / eval_U_K
            self.metrics[metric_name]["FN"][eval_name] = eval_FMD_r / eval_U_K
            self.metrics[metric_name]["TN"][eval_name] = eval_FMD_w / eval_U_K
            self.metrics[metric_name]["FP"][eval_name] = (
                eval_U_w - eval_FMD_w
            ) / eval_U_K
            # 변수명을 간단히 함
            TP_eval_name = self.metrics[metric_name]["TP"][eval_name]
            FN_eval_name = self.metrics[metric_name]["FN"][eval_name]
            TN_eval_name = self.metrics[metric_name]["TN"][eval_name]
            FP_eval_name = self.metrics[metric_name]["FP"][eval_name]
            # * 3.3. P, N 초기화
            self.metrics[metric_name]["P"][eval_name] = TP_eval_name + FN_eval_name
            self.metrics[metric_name]["N"][eval_name] = TN_eval_name + FP_eval_name
            # 변수명을 간단히 함
            P_eval_name = self.metrics[metric_name]["P"][eval_name]
            N_eval_name = self.metrics[metric_name]["N"][eval_name]
            # * 3.4. TPR, TNR, FNR, FPR 초기화
            self.metrics[metric_name]["TPR"][eval_name] = TP_eval_name / P_eval_name
            self.metrics[metric_name]["TNR"][eval_name] = TN_eval_name / N_eval_name
            self.metrics[metric_name]["FNR"][eval_name] = FN_eval_name / P_eval_name
            self.metrics[metric_name]["FPR"][eval_name] = FP_eval_name / N_eval_name
            # * 3.5. PPV, NPV, FDR, FOR 초기화
            # self.TP[eval_name] + self.FP[eval_name]가 0이 되는 경우가 발생할 때 그것으로 나누지 않음
            # 초기화된 -1 값을 그대로 가지고 감
            if TP_eval_name + FP_eval_name > 0:
                self.metrics[metric_name]["PPV"][eval_name] = TP_eval_name / (
                    TP_eval_name + FP_eval_name
                )
                self.metrics[metric_name]["FDR"][eval_name] = FP_eval_name / (
                    TP_eval_name + FP_eval_name
                )
            if FN_eval_name + TN_eval_name > 0:
                self.metrics[metric_name]["NPV"][eval_name] = TN_eval_name / (
                    FN_eval_name + TN_eval_name
                )
                self.metrics[metric_name]["FOR"][eval_name] = FN_eval_name / (
                    FN_eval_name + TN_eval_name
                )
    ...
    def set_fmdcs_infos(self, metric_name):
        ...
        for eval_name in self.eval_names:
            ...
            fmdc_min = eval_fmds.min()
            fmdc_max = eval_fmds.max()
            fmdc_interval_length = (fmdc_max - fmdc_min) / density
            self.metrics[metric_name]["fmdcs"][eval_name] = np.array(
                [
                    fmdc_min + interval_offset * fmdc_interval_length
                    for interval_offset in range(density + 1)
                ]
            )
            ...
            eval_U = self.eval_U[eval_name]
            eval_U_K = self.eval_K[eval_name]
            eval_U_r = len(np.nonzero(eval_U)[0])
            eval_U_w = eval_U_K - eval_U_r
            eval_fmds = self.metrics[metric_name]["eval_fmds"][eval_name]
            ...
            for fmdc in self.metrics[metric_name]["fmdcs"][eval_name]:
                ...
                TP = (eval_U_r - eval_FMD_r) / eval_U_K
                FN = eval_FMD_r / eval_U_K
                TN = eval_FMD_w / eval_U_K
                FP = (eval_U_w - eval_FMD_w) / eval_U_K
                P = TP + FN
                N = TN + FP
                TPR = TP / P
                TNR = TN / N
                ...
                self.metrics[metric_name]["TP_fmdc"][eval_name].append(TP)
                self.metrics[metric_name]["FN_fmdc"][eval_name].append(FN)
                self.metrics[metric_name]["TN_fmdc"][eval_name].append(TN)
                self.metrics[metric_name]["FP_fmdc"][eval_name].append(FP)
                self.metrics[metric_name]["P_fmdc"][eval_name].append(P)
                self.metrics[metric_name]["N_fmdc"][eval_name].append(N)
                self.metrics[metric_name]["TPR_fmdc"][eval_name].append(TPR)
                self.metrics[metric_name]["TNR_fmdc"][eval_name].append(TNR)
                self.metrics[metric_name]["PPV_fmdc"][eval_name].append(PPV)
                self.metrics[metric_name]["NPV_fmdc"][eval_name].append(NPV)
                self.metrics[metric_name]["FNR_fmdc"][eval_name].append(FNR)
                self.metrics[metric_name]["FPR_fmdc"][eval_name].append(FPR)
                self.metrics[metric_name]["FDR_fmdc"][eval_name].append(FDR)
                self.metrics[metric_name]["FOR_fmdc"][eval_name].append(FOR)

            # * 2.2 CM_infos 모두 넘파이 배열로 초기화
            self.metrics[metric_name]["TP_fmdc"][eval_name] = np.array(
                self.metrics[metric_name]["TP_fmdc"][eval_name]
            )
            self.metrics[metric_name]["FN_fmdc"][eval_name] = np.array(
                self.metrics[metric_name]["FN_fmdc"][eval_name]
            )
            self.metrics[metric_name]["TN_fmdc"][eval_name] = np.array(
                self.metrics[metric_name]["TN_fmdc"][eval_name]
            )
            self.metrics[metric_name]["FP_fmdc"][eval_name] = np.array(
                self.metrics[metric_name]["FP_fmdc"][eval_name]
            )
            self.metrics[metric_name]["P_fmdc"][eval_name] = np.array(
                self.metrics[metric_name]["P_fmdc"][eval_name]
            )
            self.metrics[metric_name]["N_fmdc"][eval_name] = np.array(
                self.metrics[metric_name]["N_fmdc"][eval_name]
            )
            self.metrics[metric_name]["TPR_fmdc"][eval_name] = np.array(
                self.metrics[metric_name]["TPR_fmdc"][eval_name]
            )
            self.metrics[metric_name]["TNR_fmdc"][eval_name] = np.array(
                self.metrics[metric_name]["TNR_fmdc"][eval_name]
            )
            self.metrics[metric_name]["PPV_fmdc"][eval_name] = np.array(
                self.metrics[metric_name]["PPV_fmdc"][eval_name]
            )
            self.metrics[metric_name]["NPV_fmdc"][eval_name] = np.array(
                self.metrics[metric_name]["NPV_fmdc"][eval_name]
            )
            self.metrics[metric_name]["FNR_fmdc"][eval_name] = np.array(
                self.metrics[metric_name]["FNR_fmdc"][eval_name]
            )
            self.metrics[metric_name]["FPR_fmdc"][eval_name] = np.array(
                self.metrics[metric_name]["FPR_fmdc"][eval_name]
            )
            self.metrics[metric_name]["FDR_fmdc"][eval_name] = np.array(
                self.metrics[metric_name]["FDR_fmdc"][eval_name]
            )
            self.metrics[metric_name]["FOR_fmdc"][eval_name] = np.array(
                self.metrics[metric_name]["FOR_fmdc"][eval_name]
            )
            ...
            for i in range(len(self.metrics[metric_name]["TNR_fmdc"][eval_name]) - 1):
                TPR_middle_interval_i = (
                    self.metrics[metric_name]["TPR_fmdc"][eval_name][i + 1]
                    + self.metrics[metric_name]["TPR_fmdc"][eval_name][i]
                ) / 2
                one_minus_TNR_length_interval_i = (
                    1 - self.metrics[metric_name]["TNR_fmdc"][eval_name][i + 1]
                ) - (
                    1 - self.metrics[metric_name]["TNR_fmdc"][eval_name][i]
                )  # 1-TNR = FPR
    ...
    def eval(self):
        ...
        for metric_name in self.metric_names:
            ...
            if os.path.isfile(f"{self.class_dir}/metrics/{metric_name}.pickle"):
                with open(
                    ...
                    self.metrics[metric_name]["is_eval_FMD"] = metric["is_eval_FMD"]
                    self.metrics[metric_name]["eval_fmds"] = metric["eval_fmds"]
                    self.metrics[metric_name]["TP"] = metric["TP"]
                    self.metrics[metric_name]["FN"] = metric["FN"]
                    self.metrics[metric_name]["TN"] = metric["TN"]
                    self.metrics[metric_name]["FP"] = metric["FP"]
                    self.metrics[metric_name]["N"] = metric["N"]
                    self.metrics[metric_name]["P"] = metric["P"]
                    self.metrics[metric_name]["TPR"] = metric["TPR"]
                    self.metrics[metric_name]["TNR"] = metric["TNR"]
                    self.metrics[metric_name]["PPV"] = metric["PPV"]
                    self.metrics[metric_name]["NPV"] = metric["NPV"]
                    self.metrics[metric_name]["FNR"] = metric["FNR"]
                    self.metrics[metric_name]["FPR"] = metric["FPR"]
                    self.metrics[metric_name]["FDR"] = metric["FDR"]
                    self.metrics[metric_name]["FOR"] = metric["FOR"]
                    # * 1.1.2. fmdcs_infos 초기화하기
                    self.metrics[metric_name]["fmdcs"] = metric["fmdcs"]
                    self.metrics[metric_name]["TP_fmdc"] = metric["TP_fmdc"]
                    self.metrics[metric_name]["FN_fmdc"] = metric["FN_fmdc"]
                    self.metrics[metric_name]["TN_fmdc"] = metric["TN_fmdc"]
                    self.metrics[metric_name]["FP_fmdc"] = metric["FP_fmdc"]
                    self.metrics[metric_name]["N_fmdc"] = metric["N_fmdc"]
                    self.metrics[metric_name]["P_fmdc"] = metric["P_fmdc"]
                    self.metrics[metric_name]["TPR_fmdc"] = metric["TPR_fmdc"]
                    self.metrics[metric_name]["TNR_fmdc"] = metric["TNR_fmdc"]
                    self.metrics[metric_name]["PPV_fmdc"] = metric["PPV_fmdc"]
                    self.metrics[metric_name]["NPV_fmdc"] = metric["NPV_fmdc"]
                    self.metrics[metric_name]["FNR_fmdc"] = metric["FNR_fmdc"]
                    self.metrics[metric_name]["FPR_fmdc"] = metric["FPR_fmdc"]
                    self.metrics[metric_name]["FDR_fmdc"] = metric["FDR_fmdc"]
                    self.metrics[metric_name]["FOR_fmdc"] = metric["FOR_fmdc"]
        ...
        # * 2.2. class_infos 파일이 존재하지 않는다면 지역변수 class_infos을 파일로 저장한다.
            ...
            with open(f"{self.class_dir}/class_infos.pickle", "wb") as class_infos_file:
                ...
                class_infos["class_dir"] = self.class_dir
                class_infos["origin_dir"] = self.origin_dir
                class_infos["train_dir"] = self.train_dir
                class_infos["rvalid_dir"] = self.rvalid_dir
                class_infos["wvalid_dir"] = self.wvalid_dir
                class_infos["eval_dir"] = self.eval_dir
                # * 2. data_infos를 class_infos에 넣기
                class_infos["origin_names"] = self.origin_names
                class_infos["origin_K"] = self.origin_K
                class_infos["eval_names"] = self.eval_names
                class_infos["eval_K"] = self.eval_K
                class_infos["L"] = self.L
                class_infos["shape"] = self.shape
                class_infos["FMP_count"] = self.FMP_count
                class_infos["eval_U"] = self.eval_U
                # * 3. fixed_FM_repres를 class_infos에 넣기
                class_infos["TFM_repre"] = self.TFM_repre
                class_infos["RFM_repre"] = self.RFM_repre
                class_infos["WFM_repre"] = self.WFM_repre
                # * 4. fixed_alpha_infos를 class_infos에 넣기
                class_infos["alpha_min"] = self.alpha_min
                class_infos["alpha_max"] = self.alpha_max
                class_infos["rmw_min"] = self.rmw_min
                class_infos["rmw_max"] = self.rmw_max
                # * 5. fixed_layer_infos를 class_infos에 넣기
                class_infos["norm_min"] = self.norm_min
                class_infos["norm_max"] = self.norm_max
        ...
        for metric_name in self.metric_names:
            ...
            # * 3.2. metric_name 파일이 존재하지 않는다면 metric_name을 파일로 저장한다.
                ...
                with open(
                    f"{self.class_dir}/metrics/{metric_name}.pickle", "wb"
                ) as metric_file:
    ...
    def show_square_NPs(self, nps, np_names=[], color_map=True, title_fontsize=12):
        ...
        if color_map == True:
            ...
            # 그게 아니라면 마지막에 넘파이에 nps_slice로 잘린 연속적인 값들을 넣음.
                ...
                np_min_to_max = np.array(
                    [nps_min + i * nps_interval for i in range(nps_slice + 1)]
                )
        ...
        for ith in range(len(nps)):
            ...
            x_ith = x_ith[: element_count[ith]]
            y_ith = y_ith[: element_count[ith]]
    ...
    def show_dir_infos(self):
        ...
        data_set_name = class_dir_splited[-2]
        class_name = class_dir_splited[-1]
    ...
    def show_data_infos(self):
        ...
        eval_U = [
            self.eval_U[eval_name] for eval_name in self.eval_names
        ]  # eval_U 초기화
        eval_U_name = [
            "eval_U_" + eval_name for eval_name in self.eval_names
        ]  # eval_U_name 초기화
    ...
    def show_FM_repres(
        self,
        FM_repre_HP="FM_mean",
        alpha_HP=["rmw_max", 1000],
        DAM_HP="all",
        lfmd_HP="se_lfmd",
        W_HP="C",
        fmdc_HP="rvalid_fmds_average",
    ):

        metric_name = self.get_metric_name(
            FM_repre_HP=FM_repre_HP,
            alpha_HP=alpha_HP,
            DAM_HP=DAM_HP,
            lfmd_HP=lfmd_HP,
            W_HP=W_HP,
        ...
        for FM_repre_HP in FM_repre_HPs:
            ...
            TFM_repre = self.TFM_repre[FM_repre_HP].copy()
            TFM_repre_name = [f"T{FM_repre_HP}_" + str(l) for l in range(self.L)]
            RFM_repre = self.RFM_repre[FM_repre_HP].copy()
            RFM_repre_name = [f"R{FM_repre_HP}_" + str(l) for l in range(self.L)]
            WFM_repre = self.WFM_repre[FM_repre_HP].copy()
            WFM_repre_name = [f"W{FM_repre_HP}_" + str(l) for l in range(self.L)]
            # * FM_repre, FM_repre_name 이차 정방 행렬로 출력
            self.show_square_NPs(TFM_repre, TFM_repre_name, title_fontsize=40)
            self.show_square_NPs(RFM_repre, RFM_repre_name, title_fontsize=40)
            self.show_square_NPs(WFM_repre, WFM_repre_name, title_fontsize=40)

    def show_alpha_infos(
        self,
        FM_repre_HP="FM_mean",
        alpha_HP=["rmw_max", 1000],
        DAM_HP="all",
        ...
        # * 특정 metric가 있다면 alpha_slice, alpha_min, alpha, alpha_max, rmw 출력
            ...
            for l in range(self.L):
                print("|", sep="", end="")
                print(
                    f"{self.metrics[metric_name]['alpha_percent'][l]: 9d}",
                    sep="",
                    end="",
                )
                print("%", sep="", end="")
            ...
            for l in range(self.L):
                print("|", sep="", end="")
                print(
                    f"{self.metrics[metric_name]['rmw_percent'][l]: 9d}", sep="", end=""
                )
                print("%", sep="", end="")
            print("|", sep="", end="")
            print()
    ...
    def show_AMs(
        ...
        self.set_square_NPs_infos([60, 60], column=7)
        # * AMs, AM_name 초기화
        TAM = self.metrics[metric_name]["TAM"].copy()
        TAM_name = ["TAM_" + str(l) for l in range(self.L)]
        RAM = self.metrics[metric_name]["RAM"].copy()
        RAM_name = ["RAM_" + str(l) for l in range(self.L)]
        WAM = self.metrics[metric_name]["WAM"].copy()
        WAM_name = ["WAM_" + str(l) for l in range(self.L)]
        DAM_error_flag = self.metrics[metric_name]["DAM_error_flag"]
        DAM = self.metrics[metric_name]["DAM"].copy()
        DAM_name = ["DAM_" + str(l) + f"({DAM_error_flag[l]})" for l in range(self.L)]
        # * AM_repres, AM_repre_names 이차 정방 행렬로 출력
        self.show_square_NPs(TAM, TAM_name, title_fontsize=40)
        self.show_square_NPs(RAM, RAM_name, title_fontsize=40)
        self.show_square_NPs(WAM, WAM_name, title_fontsize=40)
    ...
    def show_layer_infos(
        ...
        for l in range(self.L):
            print(
                f"{self.metrics[metric_name]['W'][l]: 7.4f}".lstrip() + " ",
                sep="",
                end="",
            )
        print()
    ...
    def show_rvalid_fmds_wvalid_fmds(
        self,
        FM_repre_HP="FM_mean",
        alpha_HP=["rmw_max", 1000],
        DAM_HP="all",
        ...
        for eval_name in self.eval_names:
            ...
            reval_fmds = eval_fmds[self.eval_U[eval_name]]
            reval_fmds_max = reval_fmds.max()
            value_names.append([reval_fmds_max, f"reval_fmds_max_{eval_name}"])
            weval_fmds = eval_fmds[np.logical_not(self.eval_U[eval_name])]
            weval_fmds_min = weval_fmds.min()
            value_names.append([weval_fmds_min, f"weval_fmds_min_{eval_name}"])
        ...
        if show_HP_fmdcs == True:
            for value, name in value_names:
                if (
                    name == "rvalid_fmds_max_wvalid_fmds_min_average"
                    or name == "rvalid_fmds_max_wvalid_fmds_min_average(fmdc)"
                ):
                    print(f"{name}:\t{value: 0.4f}")
                elif (
                    name.split("_")[-1] == "average(fmdc)"
                    or name.split("_")[-1] == "middle(fmdc)"
                ):
                    print(f"{name}:\t\t\t{value: 0.4f}")
                else:
                    print(f"{name}:\t\t\t\t{value: 0.4f}")
    ...
    def show_fmdc_infos(
        ...
        self.show_rvalid_fmds_wvalid_fmds(
            metric_name, show_category=False, show_HP_fmdcs=False
        )

        # fmdc_HP, fmdc 출력
        print(
            f"fmdc_HP, fmdc:\t{self.metrics[metric_name]['fmdc_HP']}:\t{self.metrics[metric_name]['fmdc']: 0.4f}"
        )
        # HP_fmdcs 출력하기
        HP_fmdcs = self.metrics[metric_name]["HP_fmdcs"]
        print(f"HP_fmdcs:")
    ...
    def show_eval_venn_diagram(
        ...
        plt.text(
            x=-75,
            y=40,
            s=f"TP: {self.metrics[metric_name]['TP'][eval_name]}",
            fontdict={"color": "purple", "size": 16},
        )
        plt.text(
            x=-50,
            y=0,
            s=f"FN: {self.metrics[metric_name]['FN'][eval_name]}",
            fontdict={"color": "purple", "size": 16},
        )
        plt.text(
            x=25,
            y=0,
            s=f"TN: {self.metrics[metric_name]['TN'][eval_name]}",
            fontdict={"color": "purple", "size": 16},
        )
        plt.text(
            x=25,
            y=40,
            s=f"FP: {self.metrics[metric_name]['FP'][eval_name]}",
            fontdict={"color": "purple", "size": 16},
        )
    ...
    def show_efficiency_and_FMD_ratio(
        self,
        FM_repre_HP="FM_mean",
        alpha_HP=["rmw_max", 1000],
        DAM_HP="all",
        ...
        # * 2. efficience 출력
        print(
            f"[wrong ratio U, N]:\t\t\t{self.metrics[metric_name]['N'][eval_name]: 0.4f}"
        )  # 오분류 비율 U
        print(
            f"[wrong ratio FMD, NPV]:\t\t\t{self.metrics[metric_name]['NPV'][eval_name]: 0.4f}"
        )  # 오분류 비율 FMD
        print(
            f"[recall, TPR]:\t\t\t\t{self.metrics[metric_name]['TPR'][eval_name]: 0.4f}"
        )  # 재현율(TPR)
        print(
            f"[specificity, TNR]:\t\t\t{self.metrics[metric_name]['TNR'][eval_name]: 0.4f}"
        )  # 특이도(TNR)
        print()
        # * 3. FMD ratio 출력
        eval_U_size = len(self.eval_U[eval_name])
        eval_FMD_size = len(
            self.eval_U[eval_name][self.metrics[metric_name]["is_eval_FMD"][eval_name]]
        )
        print(f"[FMD ratio(|eval_FMD|/|eval_U|)]:\t{eval_FMD_size/eval_U_size: 0.4f}")

    def show_eval_U_predicted_matched(
        self,
        FM_repre_HP="FM_mean",
        alpha_HP=["rmw_max", 1000],
        DAM_HP="all",
        ...
        self.set_square_NPs_infos([20, 10])
        eval_U = self.eval_U[eval_name]
        eval_U_predicted = np.logical_not(
            self.metrics[metric_name]["is_eval_FMD"][eval_name]
        )
        eval_U_matched = np.logical_not(np.logical_xor(eval_U, eval_U_predicted))
        self.show_square_NPs(
            [eval_U, eval_U_predicted, eval_U_matched],
            ["eval_U", "eval_U_predicted", "eval_U_matched"],
        )
        # * 3. accuracy 출력하기
        # TP, FN, TN, FP 초기화
        TP = self.metrics[metric_name]["TP"][eval_name]
        FN = self.metrics[metric_name]["FN"][eval_name]
        TN = self.metrics[metric_name]["TN"][eval_name]
        FP = self.metrics[metric_name]["FP"][eval_name]
        accuracy = (TP + TN) / (TP + FN + TN + FP)
        print(f"accuracy:\t{accuracy}")

    def show_eval_fmd_right_ratio(
        self,
        FM_repre_HP="FM_mean",
        alpha_HP=["rmw_max", 1000],
        DAM_HP="all",
        ...
        eval_fmd_min = eval_fmds.min()
        eval_fmd_max = eval_fmds.max()  # eval_fmds 최소값, 최대값 찾음.
        ...
        for interval_offset in range(eval_fmd_slice):
            ...
            interval_min = eval_fmd_min + interval_offset * eval_fmd_interval_length
            interval_max = (
                eval_fmd_min + (interval_offset + 1) * eval_fmd_interval_length
            )
            ...
            interval_values_maker = np.logical_and(
                upper_than_interval_min, lower_than_interval_max
            )
            is_right_interval_values = self.eval_U[eval_name][interval_values_maker]
            R = len(np.nonzero(is_right_interval_values)[0])
            W = len(is_right_interval_values) - R
        ...
        eval_fmd = np.array(eval_fmd)
        right_ratio = np.array(right_ratio)
        ...
        # * 4. 피어슨 상관 계수 표시하기
        x_mean = x.mean()
        y_mean = y.mean()  # x, y 평균 구하기
        cov = ((x - x_mean) * (y - y_mean)).sum() / (
            len(x) - 1
        )  # x, y 공분산 구하기, len(x) 대신 len(x)-1로
        std_x = np.power(
            ((x - x_mean) ** 2).sum() / (len(x) - 1), 1 / 2
        )  # x 표준편차 구하기
        std_y = np.power(
            ((y - y_mean) ** 2).sum() / (len(y) - 1), 1 / 2
        )  # y 표준편차 구하기
        r = cov / (std_x * std_y)  # 피어슨 상관 계수 구하기
        # line의 1/3 지점에 피어슨 상관 계수 표시하기
        plt.text(
            x=(x.min() * 2 / 3 + x.max() * 1 / 3),
            y=1,
            s=f"pearson correlation coefficient  = {r: 0.4f}",
        )
    ...
    def show_fmds(
        ...
        plt.xticks(
            [1, 2, 3, 4, 5],
            ["rvalid_fmds", "wvalid_fmds", "eval_fmds", "reval_fmds", "weval_fmds"],
        )
        ...
        for value, name in value_names:
            if (
                name == f"reval_fmds_max_{eval_name}"
                or name == f"weval_fmds_min_{eval_name}"
            ):
                plt.plot(
                    [1, 5],
                    [value, value],
    ...
    def show_effectiveness(
        ...
        p1 = plt.bar(
            index - (bar_width) / 2,
            self.metrics[metric_name]["N"][eval_name],
            bar_width,
            color="blue",
            alpha=alpha,
            label="U effectiveness",
        )
        p2 = plt.bar(
            index + (bar_width) / 2,
            self.metrics[metric_name]["NPV"][eval_name],
            bar_width,
            color="orange",
            alpha=alpha,
            label="FMD effectiveness",
        )
    ...
    def show_roc_curve(
        ...
        # * 2. ROC, AUC 그리기
        TPRs = self.metrics[metric_name]["TPR_fmdc"][eval_name]
        TNRs = self.metrics[metric_name]["TNR_fmdc"][eval_name]
        AUC = self.metrics[metric_name]["AUC"][eval_name]
        # ROC 그리기
        plt.title("ROC curve")
        ...
        for i in range(len(TPRs)):
            AUC_plot_x.append(1 - TNRs[i])
            AUC_plot_x.append(1 - TNRs[i])
            AUC_plot_y.append(0)
            AUC_plot_y.append(TPRs[i])
        plt.plot(
            AUC_plot_x, AUC_plot_y, label="AUC", color="red", alpha=0.4, linewidth=10
        )
    ...
    def show_CM_info_fmdc(
        ...
        for CM_info_name in CM_info_names:
            if CM_info_name == "P" or CM_info_name == "N":
                plt.plot(
                    fmdcs,
                    self.metrics[metric_name][f"{CM_info_name}_fmdc"][eval_name],
                    label=f"{CM_info_name}",
                    linestyle="--",
                    linewidth=4,
                    alpha=0.4,
                )
        ...
        CM_info_labels = [
            "P(U right ratio)",
            "N(U wrong ratio, U efficiency)",
            "PPV",
            "NPV(FMD wrong ratio, FMD efficiency)",
            "FDR",
            "FOR(FMD right ratio)",
        ]
    ...
    def show_all_eval_venn_diagram(
        self,
        class_dirs,
        FM_repre_HP="FM_mean",
        alpha_HP=["rmw_max", 1000],
        ...
        def show_eval_venn_diagram(
            ...
            plt.title(
                f"{class_name}", position=(0.5, 1.0 + 0.1), fontdict={"size": "32"}
            )
            ...
            plt.text(
                x=-75,
                y=40,
                s=f"TP: {metric['TP'][eval_name]}",
                fontdict={"color": "purple", "size": 16},
            )
            plt.text(
                x=-50,
                y=0,
                s=f"FN: {metric['FN'][eval_name]}",
                fontdict={"color": "purple", "size": 16},
            )
            plt.text(
                x=25,
                y=0,
                s=f"TN: {metric['TN'][eval_name]}",
                fontdict={"color": "purple", "size": 16},
            )
            plt.text(
                x=25,
                y=40,
                s=f"FP: {metric['FP'][eval_name]}",
                fontdict={"color": "purple", "size": 16},
            )
        ...
        for i, class_dir in enumerate(class_dirs):
            if os.path.isfile(f"{class_dir}/class_infos.pickle") and os.path.isfile(
                f"{class_dir}/metrics/{metric_name}.pickle"
            ):
                pass
            else:
    ...
    def show_all_eval_U_predicted_matched(
        self,
        class_dirs,
        FM_repre_HP="FM_mean",
        alpha_HP=["rmw_max", 1000],
        ...
        self.show_all_eval_U_predicted(
            class_dirs,
            FM_repre_HP=FM_repre_HP,
            alpha_HP=alpha_HP,
            DAM_HP=DAM_HP,
    ...
    def show_all_eval_fmd_right_ratio(
        self,
        class_dirs,
        FM_repre_HP="FM_mean",
        alpha_HP=["rmw_max", 1000],
        ...
        ylabel_fontsize=24,
        yticks_fontsize=16,
        labelname_fontsize=24,
        correlation_fontsize=24,
    ):
        ...
        def show_eval_fmd_right_ratio(
            ...
            for interval_offset in range(eval_fmd_slice):
                ...
                is_right_interval_values = class_infos["eval_U"][eval_name][
                    interval_values_maker
                ]
                R = len(np.nonzero(is_right_interval_values)[0])
                W = len(is_right_interval_values) - R
            ...
            plt.text(
                ...
                s=f"{class_name}",
                fontdict={"size": f"{labelname_fontsize}"},
            )  # ! 아마도 삭제
            plt.text(
                x=(x.min() + x.max()) / 2,
                y=0.85,
                horizontalalignment="center",
                s=f"pearson correlation coefficient\n= {r: 0.4f}",
                fontdict={"size": f"{correlation_fontsize}"},
            )
    ...
    def show_all_fmds(
        ...
        def show_fmds(
            ...
            # reval_fmds: 정분류 평가 데이터에 대한 fmds, weval_fmds: 오분류 평가 데이터에 대한 fmds
            reval_fmds = eval_fmds[class_infos["eval_U"][eval_name]]
            weval_fmds = eval_fmds[np.logical_not(class_infos["eval_U"][eval_name])]
            plt.boxplot(
                [
                    metric["rvalid_fmds"],
                    metric["wvalid_fmds"],
    ...
    def show_all_fmdc_TNR_TPR(
        ...
        def show_fmdc_TNR_TPR(
            ...
            for fmd in sorted_eval_fmds:
                if fmd not in fmds:
                    upper_than_fmd_reval = reval_fmds >= fmd
                    upper_than_fmd_wvalid = weval_fmds >= fmd
                    selected_reval_fmds = reval_fmds[upper_than_fmd_reval]
                    selected_weval_fmds = weval_fmds[upper_than_fmd_wvalid]
                    selected_reval_count = len(selected_reval_fmds)
                    selected_weval_count = len(selected_weval_fmds)
                    unselected_reval_count = len(reval_fmds) - selected_reval_count
                    unselected_weval_count = len(weval_fmds) - selected_weval_count

                    TNR = selected_weval_count / (
                        unselected_weval_count + selected_weval_count
                    )
                    recall = TNR
                    TPR = unselected_reval_count / (
                        unselected_reval_count + selected_reval_count
                    )

                    if selected_reval_count + selected_weval_count == 0:
                        precision = -1
                    else:
                        precision = selected_weval_count / (
                            selected_reval_count + selected_weval_count
                        )
            ...
            fmds_to_be_plotted = []
            f1_scores_to_be_plotted = []
            TNRs_to_be_plotted = []
            TPRs_to_be_plotted = []
            for i in range(len(fmds) - 1):
                fmds_to_be_plotted.append(fmds[i])
                fmds_to_be_plotted.append(fmds[i + 1])
                f1_scores_to_be_plotted.append(f1_scores[i])
                f1_scores_to_be_plotted.append(f1_scores[i])
                TNRs_to_be_plotted.append(TNRs[i])
                TNRs_to_be_plotted.append(TNRs[i])
                TPRs_to_be_plotted.append(TPRs[i])
                TPRs_to_be_plotted.append(TPRs[i])
            ...
            plt.xticks(fontsize=xticks_fontsize)
            plt.xlabel(
                "feature map distance criteria(threshold, fmdc)",
                {"size": f"{xlabel_fontsize}"},
            )
            plt.yticks(fontsize=yticks_fontsize)
        ...
        for label in labels:
            if label == "TPR" and show_tpr == True:
                ...
                if label in fmdc_names or label[:-6] in fmdc_names:
                    if (
                        label == "rvalid_fmds_average"
                        or label[:-6] == "rvalid_fmds_average"
                    ):
                        plt.plot(
                            1,
                            1,
                            label="Average of correctly classified validation data fmd",
                        )
                    elif (
                        label == "wvalid_fmds_average"
                        or label[:-6] == "wvalid_fmds_average"
                    ):
                        plt.plot(
                            1,
                            1,
                            label="Average of incorrectly classified validation data fmd",
                        )
                    elif (
                        label == "wvalid_fmds_middle"
                        or label[:-6] == "wvalid_fmds_middle"
                    ):
                        plt.plot(
                            1,
                            1,
                            label="Median of incorrectly classified validation data fmd",
                        )
                    else:
                        plt.plot(1, 1, label=label)
    ...
    def show_all_roc_curve(
        ...
        def show_roc_curve(
            ...
            # * 2. ROC, AUC 그리기
            TPRs = metric["TPR_fmdc"][eval_name]
            TNRs = metric["TNR_fmdc"][eval_name]
            AUC = metric["AUC"][eval_name]
            # ROC 그리기
            plt.plot(1 - TNRs, TPRs)
            # AUC 그리기
            AUC_plot_x = []
    ...
    def show_all_P_N_TP_FN_TN_FP_fmdc(
        self,
        class_dirs,
        FM_repre_HP="FM_mean",
        alpha_HP=["rmw_max", 1000],
        ...
        def show_P_N_TP_FN_TN_FP_fmdc(
            ...
            for CM_info_name in CM_info_names:
                if CM_info_name == "P" or CM_info_name == "N":
                    plt.plot(
                        fmdcs,
                        metric[f"{CM_info_name}_fmdc"][eval_name],
                        label=f"{CM_info_name}",
                        linestyle="--",
                        linewidth=4,
                        alpha=0.4,
    ...
    def show_all_TPR_TNR_FNR_FPR_fmdc(
        self,
        class_dirs,
        FM_repre_HP="FM_mean",
        alpha_HP=["rmw_max", 1000],
    ...
    def show_all_P_N_PPV_NPV_FDR_FOR_fmdc(
        self,
        class_dirs,
        FM_repre_HP="FM_mean",
        alpha_HP=["rmw_max", 1000],
        ...
        def show_P_N_PPV_NPV_FDR_FOR_fmdc(
            subplot_index,
            class_dir,
            FM_repre_HP="FM_mean",
            alpha_HP=["rmw_max", 1000],
    ...
    def show_all_fmdcs_infos(
        ...
        self.show_all_P_N_TP_FN_TN_FP_fmdc(
            class_dirs,
            FM_repre_HP=FM_repre_HP,
            alpha_HP=alpha_HP,
            DAM_HP=DAM_HP,
        ...
        self.show_all_TPR_TNR_FNR_FPR_fmdc(
            class_dirs,
            FM_repre_HP=FM_repre_HP,
            alpha_HP=alpha_HP,
            DAM_HP=DAM_HP,
        ...
        self.show_all_P_N_PPV_NPV_FDR_FOR_fmdc(
            class_dirs,
            FM_repre_HP=FM_repre_HP,
            alpha_HP=alpha_HP,
            DAM_HP=DAM_HP,
    ...
    def show_all_correlation_table(
        self,
        class_dirs,
        FM_repre_HP="FM_mean",
        alpha_HP=["rmw_max", 1000],
    ...
    def show_all_HP_fmdc_table(
        ...
        for i, class_dir in enumerate(class_dirs):
            ...
            values_i = np.empty((9, 1), dtype="<U16")
            HP_fmdcs = metric["HP_fmdcs"]
            ...
            reval_fmds = eval_fmds[class_infos["eval_U"][eval_name]]
            reval_fmds_max = reval_fmds.max()
            values_i[7][0] = f"{reval_fmds_max: 0.2f}".strip()
            weval_fmds = eval_fmds[np.logical_not(class_infos["eval_U"][eval_name])]
            weval_fmds_min = weval_fmds.min()
            values_i[8][0] = f"{weval_fmds_min: 0.2f}".strip()
    ...
    def show_all_fmd_ratio_table(
        ...
        for i, class_dir in enumerate(class_dirs):
            ...
            # [FN + TN: FMD ratio]
            FN = metric["FN"][eval_name]
            TN = metric["TN"][eval_name]
    ...
    def show_all_effectiveness(
        ...
        for i, class_dir in enumerate(class_dirs):
            ...
            class_dir_splited = class_infos["class_dir"].split("/")
            class_name = class_dir_splited[-1]
            p1 = plt.bar(
                i - (bar_width) / 2,
                metric["N"][eval_name],
        ...
        plt.legend(
            (p1[0], p2[0]),
            ("U effectiveness", "FMD effectiveness"),
    ...
    def show_all_effectiveness_table(
        self,
        class_dirs,
        FM_repre_HP="FM_mean",
        alpha_HP=["rmw_max", 1000],
        ...
        index_names = []
        # index_names에 추가
        index_names.append("U count")
        index_names.append("Wrong U count")
        index_names.append("U Effectiveness")
        index_names.append("FMD count")
        index_names.append("Wrong FMD count")
        index_names.append("FMD Effectiveness")
        index_names.append("Effectiveness distance difference")
        index_names.append("Effectiveness proportion difference")
        ...
        for i, class_dir in enumerate(class_dirs):
            ...
            # U, FMD에 관한 값을 넣음
            eval_U = class_infos["eval_U"][eval_name]
            is_eval_FMD = metric["is_eval_FMD"][eval_name]
            U_K = len(eval_U)
            U_r = len(np.nonzero(eval_U)[0])
            U_w = U_K - U_r
            N = metric["N"][eval_name]
            values_i[0][0] = f"{U_K}"
            values_i[1][0] = f"{U_w}"
            values_i[2][0] = f"{N: 0.4f}".strip()
            FMD_K = len(eval_U[is_eval_FMD])
            FMD_r = len(np.nonzero(eval_U[is_eval_FMD])[0])
            FMD_w = FMD_K - FMD_r
            NPV = metric["NPV"][eval_name]
            values_i[3][0] = f"{FMD_K}"
            values_i[4][0] = f"{FMD_w}"
            values_i[5][0] = f"{NPV: 0.4f}".strip()
            values_i[6][0] = f"{NPV - N: 0.4f}".strip()
            values_i[7][0] = f"{((NPV - N)/N)*100: 0.4f}".strip()
        ...
        values_float_mean_str = np.empty((index_count, 1), dtype="<U16")
    ...
    def show_accuracy(
        ...
        for class_dir in class_dirs:
            ...
            eval_U = class_infos["eval_U"][eval_name]
            eval_Us.append(eval_U)
            eval_U_K = len(eval_U)
            eval_U_Ks.append(eval_U_K)
            eval_U_r = len(np.nonzero(eval_U)[0])
            eval_U_rs.append(eval_U_r)
            eval_U_w = eval_U_K - eval_U_r
            eval_U_ws.append(eval_U_w)

            is_eval_FMD = metric["is_eval_FMD"][eval_name]
            is_eval_FMDs.append(is_eval_FMD)
            eval_FMD_K = len(eval_U[is_eval_FMD])
            eval_FMD_Ks.append(eval_FMD_K)
            eval_FMD_r = len(np.nonzero(eval_U[is_eval_FMD])[0])
            eval_FMD_rs.append(eval_FMD_r)
            eval_FMD_w = eval_FMD_K - eval_FMD_r
            eval_FMD_ws.append(eval_FMD_w)
        # eval_Us, eval_U_Ks, eval_U_rs, eval_U_ws, is_eval_FMDs, eval_FMD_Ks, eval_FMD_rs, eval_FMD_wsd을 넘파이 배열로 변환
        eval_Us = np.array(eval_Us)
        eval_U_Ks = np.array(eval_U_Ks)
        eval_U_rs = np.array(eval_U_rs)
        eval_U_ws = np.array(eval_U_ws)
        is_eval_FMDs = np.array(is_eval_FMDs)
        eval_FMD_Ks = np.array(eval_FMD_Ks)
        eval_FMD_rs = np.array(eval_FMD_rs)
        eval_FMD_ws = np.array(eval_FMD_ws)

        sum_eval_U_Ks = eval_U_Ks.sum()
        sum_eval_U_rs = eval_U_rs.sum()
        sum_eval_U_ws = eval_U_ws.sum()
        sum_eval_FMD_Ks = eval_FMD_Ks.sum()
        sum_eval_FMD_rs = eval_FMD_rs.sum()
        sum_eval_FMD_ws = eval_FMD_ws.sum()
        ...
        p2 = plt.bar(
            0 + (bar_width) / 2,
            sum_eval_FMD_rs / sum_eval_FMD_Ks,
            bar_width,
            color="orange",
            alpha=alpha,
            label="FMD accuracy",
    ...
    def show_accuracy_table(
        ...
        plt.table(
            cellText=[
                [
                    f"{sum_eval_U_rs / sum_eval_U_Ks: 0.4f}",
                    f"{sum_eval_FMD_rs / sum_eval_FMD_Ks: 0.4f}",
                ]
            ],
            cellLoc="center",
            colLabels=["U accuracy", "FMD accuracy"],
            rowLabels=["accuracy"],
            loc="center",
        )
    ...
    def show_all_eval_fmd_right_count_wrong_count(
        self,
        class_dirs,
        FM_repre_HP="FM_mean",
        alpha_HP=["rmw_max", 1000],
        ...
        labelname_fontsize=24,
        percent_fontsize=16,
        percent_intervalsize=0.02,
        percent_alpha=0.4,
        percent_width=1,
        legend_fontsize=24,
        HP_fmdc_width=10,
        HP_fmdc_alpha=0.4,
        guideline_interval=10,
        guideline_width=10,
        guideline_alpha=0.4,
        show_right_count=True,
        show_wrong_count=True,
        show_right_percent=True,
        show_wrong_percent=True,
        show_guideline=True,
        show_fmdcs=True,
        show_right_percent_line=True,
        show_wrong_percent_line=True,
    ):

        def show_eval_fmd_right_count_wrong_count(
            subplot_index,
            class_dir,
            FM_repre_HP="FM_mean",
            alpha_HP=["rmw_max", 1000],
            ...
            for interval_offset in range(eval_fmd_slice):
                ...
                R_count_value = len(np.nonzero(is_right_interval_values)[0])
                W_count_value = len(is_right_interval_values) - R_count_value

                R_count.append(R_count_value)
                W_count.append(W_count_value)
                eval_fmd.append(eval_fmd_value)
            ...
            R_ones = R_count.copy()
            np.place(R_ones, R_ones > 0, 1)
            W_ones = W_count.copy()
            np.place(W_ones, W_ones > 0, 1)

            y_max = max(max(R_count), max(W_count))
            y_min = min(min(R_count), min(W_count))
            ...
            reval_fmds = eval_fmds[class_infos["eval_U"][eval_name]]
            weval_fmds = eval_fmds[np.logical_not(class_infos["eval_U"][eval_name])]
            sorted_reval_fmds = sorted(reval_fmds)
            sorted_weval_fmds = sorted(weval_fmds)
            total_size_reval_fmds = sorted_reval_fmds.__len__()
            total_size_weval_fmds = sorted_weval_fmds.__len__()
            unit_offset_reval_fmds = 100 / total_size_reval_fmds
            unit_offset_weval_fmds = 100 / total_size_weval_fmds
            previous_interval_index_reval_fmds = None
            previous_interval_index_weval_fmds = None
            current_interval_index_reval_fmds = None
            current_interval_index_weval_fmds = None
            first_point_in_interval_reval_fmds = []
            first_point_in_interval_weval_fmds = []

            for i in range(total_size_reval_fmds):
                offset = (i + 1) * unit_offset_reval_fmds
                current_interval_index_reval_fmds = offset // 5 + 1
                if (
                    current_interval_index_reval_fmds != 1
                    and previous_interval_index_reval_fmds
                    != current_interval_index_reval_fmds
                ):
                    previous_interval_index_reval_fmds = (
                        current_interval_index_reval_fmds
                    )
                    first_point_in_interval_reval_fmds.append(
                        (sorted_reval_fmds[i], offset, i, total_size_reval_fmds - i)
                    )
            for i in range(total_size_weval_fmds):
                offset = (i + 1) * unit_offset_weval_fmds
                current_interval_index_weval_fmds = offset // 5 + 1
                if (
                    current_interval_index_weval_fmds != 1
                    and previous_interval_index_weval_fmds
                    != current_interval_index_weval_fmds
                ):
                    previous_interval_index_weval_fmds = (
                        current_interval_index_weval_fmds
                    )
                    first_point_in_interval_weval_fmds.append(
                        (sorted_weval_fmds[i], offset, i, total_size_weval_fmds - i)
                    )

            for i in range(len(first_point_in_interval_reval_fmds)):
                x_value = first_point_in_interval_reval_fmds[i][0]
                offset_str = (
                    f"{int(first_point_in_interval_reval_fmds[i][1])}".strip()
                    + "\n"
                ...
                if show_right_percent_line == True:
                    plt.plot(
                        ...
                        linestyle="-",
                        alpha=percent_alpha,
                        linewidth=percent_width,
                    )
                if show_right_percent == True:
                    if i % 5 == 0:
                        plt.text(
                            x=x_value,
                            y=y_max * (0.5 + percent_intervalsize),
                            s=offset_str,
                            color="blue",
                            fontdict={"size": f"{percent_fontsize}"},
                        ...
                        plt.text(
                            x=x_value,
                            y=y_max * (0.5 + 2 * percent_intervalsize),
                            s=offset_str,
                            color="blue",
                            fontdict={"size": f"{percent_fontsize}"},
                            verticalalignment="center",
                        ...
                        plt.text(
                            x=x_value,
                            y=y_max * (0.5 + 3 * percent_intervalsize),
                            s=offset_str,
                            color="blue",
                            fontdict={"size": f"{percent_fontsize}"},
                            verticalalignment="center",
                        ...
                        plt.text(
                            x=x_value,
                            y=y_max * (0.5 + 4 * percent_intervalsize),
                            s=offset_str,
                            color="blue",
                            fontdict={"size": f"{percent_fontsize}"},
                            verticalalignment="center",
                        ...
                        plt.text(
                            x=x_value,
                            y=y_max * (0.5 + 5 * percent_intervalsize),
                            s=offset_str,
                            color="blue",
                            fontdict={"size": f"{percent_fontsize}"},
                            verticalalignment="center",
            ...
            for i in range(len(first_point_in_interval_weval_fmds)):
                ...
                offset_str = (
                    f"{int(first_point_in_interval_weval_fmds[i][1])}".strip()
                    + "\n"
                ...
                if "fmdc_" in fmdc_name:
                    ...
                    fmdcs.append(
                        [
                            (
                                first_point_in_interval_reval_fmds[
                                    -1 - rvalid_percent_index
                                ][0]
                                + first_point_in_interval_weval_fmds[
                                    wvalid_percent_index
                                ][0]
                            )
                            / 2,
                            fmdc_name,
            ...
            wvalid_fmds = metric["wvalid_fmds"]
            sorted_wvalid_fmds = sorted(wvalid_fmds)

            fmdcs.append(
                [
                    sorted_wvalid_fmds[sorted_wvalid_fmds.__len__() // 2],
                    "wvalid_fmds_middle",
                ]
            )
            ...
            for key in HP_fmdcs.keys():
                if key in fmdc_names and show_fmdcs == True:
                    plt.plot(
                        [HP_fmdcs[key], HP_fmdcs[key]],
                        [y_min, y_max],
                        label=key,
                        linewidth=HP_fmdc_width,
                        alpha=HP_fmdc_alpha,
                    )
            ...
            if show_guideline == True:
                ...
                for i in range(y_max // guideline_interval):
                    y.append(
                        [(i + 1) * guideline_interval, (i + 1) * guideline_interval]
                    )
                for y_i in y:
                    plt.plot(
                        x,
                        y_i,
                        linestyle=":",
                        linewidth=guideline_width,
                        alpha=guideline_alpha,
                        color="gray",
                    )

            plt.text(
                x=(min(eval_fmd) + max(eval_fmd)) / 2,
                y=y_max * 9 / 10 + y_min * 1 / 10,
                horizontalalignment="center",
                s=f"{class_name}",
                fontdict={"size": f"{labelname_fontsize}"},
            )  # ! 아마도 삭제
            ...
            plt.legend(
                loc="upper left", bbox_to_anchor=(1.0, 1.0), fontsize=legend_fontsize
            )
            plt.xticks(fontsize=xticks_fontsize)
            plt.xlabel("feature map distance", {"size": f"{xlabel_fontsize}"})
            plt.xlim(
                min(min(eval_fmd), min(HP_fmdc_values)),
                max(max(eval_fmd), max(HP_fmdc_values)),
            )  # interval에 아무것도 없는 것은 표하지 않음
            plt.yticks(fontsize=yticks_fontsize)
        ...
        for i, class_dir in enumerate(class_dirs):
            subplot_index = [row_count, column_count, i + 1]
            show_eval_fmd_right_count_wrong_count(
                subplot_index=subplot_index,
                class_dir=class_dir,
                FM_repre_HP=FM_repre_HP,
                alpha_HP=alpha_HP,
    ...
    def show_all_fmds_effectiveness_f1_score_recall(
        self,
        class_dirs,
        FM_repre_HP="FM_mean",
        alpha_HP=["rmw_max", 1000],
        ...
        height=9,
        column_count=5,
        effectiveness_width=10,
        effectiveness_alpha=0.4,
        xlabel_fontsize=24,
        xticks_fontsize=16,
        ylabel_fontsize=24,
        yticks_fontsize=16,
        ...
        guideline_width=10,
        guideline_alpha=0.4,
        show_fmd_effectiveness=True,
        show_u_effectiveness=True,
        show_recall=True,
        show_f1_score=True,
        show_right_percent=True,
        show_wrong_percent=True,
        ...
        show_right_percent_line=True,
        show_wrong_percent_line=True,
        show_wvalid_fmds_middle_new=True,
        show_wvalid_fmds_middle_old=True,
    ):

        legend_names = []

        def show_fmds_effectiveness_f1_score_recall(
            subplot_index,
            class_dir,
            FM_repre_HP="FM_mean",
            alpha_HP=["rmw_max", 1000],
            ...
            if show_u_effectiveness == True:
                plt.plot(
                    [min(eval_fmds), max(eval_fmds)],
                    [U_effectiveness, U_effectiveness],
                    color="black",
                    linewidth=effectiveness_width,
                    linestyle="-",
                    alpha=effectiveness_alpha,
                    label="U Effectiveness",
                )
            ...
            for fmd in sorted_eval_fmds:
                if fmd not in fmds:
                    ...
                    effectiveness = selected_weval_count / (
                        selected_reval_count + selected_weval_count
                    )
                    recall = selected_weval_count / (
                        unselected_weval_count + selected_weval_count
                    )
                    if selected_reval_count + selected_weval_count == 0:
                        precision = -1
            ...
            fmds_to_be_plotted = []
            effectivenesses_to_be_plotted = []
            recalls_to_be_plotted = []
            f1_scores_to_be_plotted = []
            precisions_to_be_plotted = []
            for i in range(len(fmds) - 1):
                fmds_to_be_plotted.append(fmds[i])
                fmds_to_be_plotted.append(fmds[i + 1])
                effectivenesses_to_be_plotted.append(effectivenesses[i])
                effectivenesses_to_be_plotted.append(effectivenesses[i])
                recalls_to_be_plotted.append(recalls[i])
                recalls_to_be_plotted.append(recalls[i])
                f1_scores_to_be_plotted.append(f1_scores[i])
                f1_scores_to_be_plotted.append(f1_scores[i])
                precisions_to_be_plotted.append(precisions[i])
                precisions_to_be_plotted.append(precisions[i])
            ...
            if (
                "wvalid_fmds_middle" in fmdc_names
                and show_wvalid_fmds_middle_new == True
                and show_fmdcs == True
            ):
                fmdcs.append(
                    [
            ...
            plt.xlabel("feature map distance", {"size": f"{xlabel_fontsize}"})
            plt.xlim(
                min(min(fmds), min(HP_fmdc_values)), max(max(fmds), max(HP_fmdc_values))
            )  # interval에 아무것도 없는 것은 표하지 않음
            plt.yticks(fontsize=yticks_fontsize)
        ...
        for i, class_dir in enumerate(class_dirs):
            subplot_index = [row_count, column_count, i + 1]
            show_fmds_effectiveness_f1_score_recall(
                subplot_index=subplot_index,
                class_dir=class_dir,
                FM_repre_HP=FM_repre_HP,
                alpha_HP=alpha_HP,
        ...
        plt.legend(
            loc="upper left", bbox_to_anchor=(0.0, 1.0), fontsize=legend_fontsize
        )
    ...
    def show_all_fmdc_recall_precision_f1_score_table(
        self,
        class_dirs,
        FM_repre_HP="FM_mean",
        alpha_HP=["rmw_max", 1000],
        ...
        def show_fmdc_recall_precision_f1_score_table(
            subplot_index,
            class_dir,
            FM_repre_HP="FM_mean",
            alpha_HP=["rmw_max", 1000],
            ...
            # 표
            column_names = [f"{class_name}", "Recall", "Precision", "F1 Score"]
            cell_values = []
            recalls = []
            precisions = []
            f1_scores = []
            for fmdc, name in fmdc_and_names:
                upper_than_fmd_reval = reval_fmds >= fmdc
                upper_than_fmd_wvalid = weval_fmds >= fmdc
                selected_reval_fmds = reval_fmds[upper_than_fmd_reval]
                selected_weval_fmds = weval_fmds[upper_than_fmd_wvalid]
                selected_reval_count = len(selected_reval_fmds)
                selected_weval_count = len(selected_weval_fmds)
        ...
        for i, class_dir in enumerate(class_dirs):
            subplot_index = [row_count, column_count, i + 1]
            show_fmdc_recall_precision_f1_score_table(
                subplot_index=subplot_index,
                class_dir=class_dir,
                FM_repre_HP=FM_repre_HP,
                alpha_HP=alpha_HP,
